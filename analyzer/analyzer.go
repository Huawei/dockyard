...
func AnalyseRegistry(dockerURL string, username string, password string, insecure bool) ([]attr.DockerImg_Attr, error) {
	repoBackend := registry.NewRepoBackend(username, password, insecure)
	return analyseReal(repoBackend, dockerURL)
}

func AnalyseLocal(dockerURL string, filePath string) ([]attr.DockerImg_Attr, error) {
...
func analyseReal(backend AnalyseBackend, dockerURL string) ([]attr.DockerImg_Attr, error) {
